...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir, scale=1, mask_suffix='_mask', tif=True):
    ...
    def __getitem__(self, i):
        ...
        mask = gdal.Open(mask_file[0], gdal.GA_ReadOnly)
        img = gdal.Open(img_file[0], gdal.GA_ReadOnly)

        # # Transpose image from (bands, rows, columns) to (rows, columns, bands)
        img = img.ReadAsArray()
