...
parser.add_argument( # inner buffer size 
    '-i', '--inner-buffer',
    type = int,
    default = general.LABEL_INNER_BUFFER,
...
parser.add_argument( # outer buffer size 
    '-u', '--outer-buffer',
    type = int,
    default = general.LABEL_OUTER_BUFFER,
...
output = os.path.join(args.output_path, f'{general.LABEL_PREFIX}_teste_{args.year}.tif')
...
gdal.RasterizeLayer(target_ds, [1], l_previous_def, burn_values=[1])
